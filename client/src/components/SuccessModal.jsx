  ...
  const AccountPendingView = ({ o }) => (
    <>
      ...
      <div className="flex items-center gap-2 rounded-[10px] border border-gold/20 bg-gold/5 px-4 py-3 text-[0.82rem] text-body/80">
        ...
        <span>
          This account is being prepared by the provider. The credentials will be delivered to{' '}
          <span className="text-gold">Paid Accounts</span> and your email within{' '}
          <CountdownTimer
            expiresAt={o.expiresAt || new Date(new Date(o.purchasedAt || Date.now()).getTime() + 10 * 60 * 1000).toISOString()}
        ...
        {isAccount && allPending ? (
          <>
            ...
            <div className="flex items-center gap-2 rounded-[10px] border border-gold/20 bg-gold/5 px-4 py-3 text-[0.82rem] text-body/80">
              <Clock size={15} strokeWidth={1.9} className="text-gold shrink-0" />
              <span>
                Your account{count > 1 ? 's are' : ' is'} being prepared by the provider. The credentials will be delivered to{' '}
                <span className="text-gold">Paid Accounts</span> and your email within{' '}
                <CountdownTimer
                  expiresAt={first.expiresAt || new Date(new Date(first.purchasedAt || Date.now()).getTime() + 10 * 60 * 1000).toISOString()}
                  className="text-gold font-semibold"
                />
              </span>
            </div>
